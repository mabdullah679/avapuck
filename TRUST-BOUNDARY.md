...
| 2.9 | **Kafka topics** | **WORKS, and deliberately narrower than the request.** The publish stage sends each row to two topics from one producer: `rpos_encrypted` carries the sensitive fields **as ciphertext**, `rpos_flat` carries the columns that were never sensitive (price, category, status, timestamps). The requirement was phrased as "the encrypted value to one topic and the unencrypted value to the other"; **taken literally that would put real names, emails, street addresses and card numbers into a durable, replicated, retained log** — a larger exposure than the warehouse, readable by anything with topic access, and a direct breach of invariant 1. The client chose the narrower reading. Which columns go where comes from the manifest, and `_assert_no_plaintext_published` refuses the send if a sensitive column is ever routed to the flat topic. **What is NOT production-grade:** the broker is single-node KRaft with `replication-factor 1` and **no auth, no ACLs and no TLS** — any container on the compose network can read either topic. Ciphertext on `rpos_encrypted` is still protected by AES; `rpos_flat` is readable by design. |
